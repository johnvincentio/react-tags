...
class ReactTags extends Component {
  ...
  getQueryIndex = (query, item) => item[this.props.labelField]
  	.toLowerCase()
  ...
  filteredSuggestions(query, suggestions) {
  	...
  	const exactSuggestions = suggestions.filter((item) => this.getQueryIndex(query, item) === 0);
  	const partialSuggestions = suggestions.filter((item) => this.getQueryIndex(query, item) > 0);
